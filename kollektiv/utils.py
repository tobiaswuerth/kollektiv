...
def save_pydantic_json(obj: BaseModel, file_name: str) -> None:
    ...
    os.makedirs(config.output_dir.directory, exist_ok=True)
...
def load_pydantic_json(file_name: str, model: type) -> BaseModel:
    path = os.path.join(config.output_dir.directory, file_name)
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"File '{file_name}' not found in '{config.output_dir.directory}'."
        )
...
def generate_project_plan_graph(json_file_path: str, output_png_path: str) -> None:
    ...
    # --- Phase Iteration ---
    previous_phase_bottom_node = (
        None  # Track last node of previous phase for vertical ordering
    )
    for i, phase in enumerate(project_plan.get("project_phases", [])):
        ...
        phase_deliverable_files = {
            d.get("file_name")
            for d in phase.get("deliverable_files", [])
            if d.get("file_name")
        }
        ...
        with dot.subgraph(name=phase_id, label=phase_name) as phase_graph:
            phase_graph.graph_attr["style"] = (
                "filled, rounded"  # Rounded corners for phase
            )
            ...
            for input_file in phase_required_inputs:
                ...
                phase_graph.add_node(
                    phase_relay_node_id,
                    label=input_file,
                    shape=SHAPE_RELAY,
                    fillcolor=COLOR_PHASE_INPUT,
                )
            ...
            for j, task in enumerate(phase.get("tasks", [])):
                ...
                task_action_node_id = (
                    f"action_{i}_{j}"  # Unique ID for the task action node
                )

                # Create subgraph for the task
                with phase_graph.subgraph(
                    name=task_subgraph_id, label=f"Task: {task_name}"
                ) as task_graph:
                    ...
                    for input_file in task_required_inputs:
                        ...
                        phase_relay_node_id = phase_input_relay_nodes.get(
                            (i, input_file)
                        )
                        if phase_relay_node_id:
                            # Create a task-specific relay node (Purple Oval) INSIDE the task subgraph
                            task_phase_relay_node_id = (
                                f"task_phase_relay_{i}_{j}_{input_file}"
                            )
                            task_graph.add_node(
                                task_phase_relay_node_id,
                                label=input_file,
                                shape=SHAPE_RELAY,
                                fillcolor=COLOR_TASK_INPUT,
                            )
                            ...
                            task_graph.add_edge(
                                task_phase_relay_node_id, task_action_node_id
                            )
                        else:
                            ...
                            if source_node_id:
                                ...
                                task_intermediate_relay_node_id = (
                                    f"task_intermediate_relay_{i}_{j}_{input_file}"
                                )
                                task_graph.add_node(
                                    task_intermediate_relay_node_id,
                                    label=input_file,
                                    shape=SHAPE_RELAY,
                                    fillcolor=COLOR_TASK_INTERMEDIATE_INPUT,
                                )

                                # Connect Source Node -> Intermediate Relay (Edge added to parent graph)
                                dot.add_edge(
                                    source_node_id, task_intermediate_relay_node_id
                                )

                                # Connect Intermediate Relay -> Task Action (Edge within task subgraph)
                                task_graph.add_edge(
                                    task_intermediate_relay_node_id, task_action_node_id
                                )
                            else:
                    ...
                    if output_file:
                        ...
                        output_color = (
                            ...
                            else COLOR_TASK_INTERMEDIATE_OUTPUT
                        )
                        ...
                        globally_added_nodes.add(
                            output_node_id
                        )  # Track that this node exists
        ...
        if previous_phase_bottom_node and phase_first_node:
            ...
            dot.add_edge(
                previous_phase_bottom_node, phase_first_node, style="invis", weight=100
            )
        if phase_last_node:
